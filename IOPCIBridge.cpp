                                       ...
                                       IOPCIAddressSpace * space )
{
    ...
    //Slice - this is from v1.6
    else if ((regProp = (OSData *) propTable->getObject("acpi-reg")))
    {
        inSpace = (IOPCIAddressSpace *) regProp->getBytesNoCopy();
        space->s.busNum = firstBusNum();//inSpace->s.busNum; //
        space->s.deviceNum = inSpace->s.deviceNum;
        space->s.functionNum = inSpace->s.functionNum;
    }
    //
...
//                || !strcmp("ethernet", nub->getName())
    ...
    for (int scanDevice = 0; scanDevice <= 31; scanDevice++)
    {
        int lastFunction = 0;
        for (int scanFunction = 0; scanFunction <= lastFunction; scanFunction++)
        {
            space = getBridgeSpace();
            ...
            if (findMatching(kidsIter, space) == 0)
            {
                ...
                continue;
#endif
                UInt32 vendor = configRead32( getBridgeSpace(), kIOPCIConfigVendorID );
                vendor &= 0x0000ffff;
...
checkCardBusNumbering(nubs);
    ...
    //    resolvePCIInterrupt( provider, nub );
        ...
        {
            if (yentaCount < 8) yentaIndices[yentaCount++] = idx;

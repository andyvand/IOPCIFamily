...
extern "C" unsigned long long ml_phys_read_double_64(addr64_t paddr);
			  ...
			  uint32_t mapOptions, upl_page_info_t * pageList)
{
	...
	if (kIODMAMapPhysicallyContiguous & mapOptions) VTLOG("map phys %lxx, %x\n", (unsigned long)ptoa_64(pageList[0].opaque), size);
	...
	if (kIODMAMapPhysicallyContiguous & mapOptions)
	{
		...
		{
			bf->tables[0][start + idx].bits = (access | ptoa_64(pageList[0].opaque + idx));
		}
#if TABLE_CB
	...
	else
	{
		...
		{
			j = 0;
			bf->tables[0][start + idx + j].bits = (access | ptoa_64(pageList[idx + j].opaque)); j++;
			bf->tables[0][start + idx + j].bits = (access | ptoa_64(pageList[idx + j].opaque)); j++;
			bf->tables[0][start + idx + j].bits = (access | ptoa_64(pageList[idx + j].opaque)); j++;
			bf->tables[0][start + idx + j].bits = (access | ptoa_64(pageList[idx + j].opaque)); j++;
			bf->tables[0][start + idx + j].bits = (access | ptoa_64(pageList[idx + j].opaque)); j++;
			bf->tables[0][start + idx + j].bits = (access | ptoa_64(pageList[idx + j].opaque)); j++;
			bf->tables[0][start + idx + j].bits = (access | ptoa_64(pageList[idx + j].opaque)); j++;
			bf->tables[0][start + idx + j].bits = (access | ptoa_64(pageList[idx + j].opaque));
			__mfence();
		...
		for (idx = 0; idx < size; idx++)
		{
			bf->tables[0][start + idx].bits = (access | ptoa_64(pageList[idx].opaque));
		}
#endif
	}
	__mfence();
					 ...
					 upl_page_info_t *pageList, IOItemCount pageCount)
{
    ...
    for (idx = 0; idx < pageCount; idx++)
    {
		fSpace->tables[0][addr + idx].bits = ptoa_64(pageList[idx].opaque) | kPageAccess;
	}
	table_flush(&fSpace->tables[0][addr], pageCount * sizeof(vtd_table_entry_t), fCacheLineSize);
	STAT_ADD(fSpace, inserts, pageCount);
}

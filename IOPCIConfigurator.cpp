...
// AnV - Fixes
#ifdef DEBUG
		fFlags |= (kIOPCIConfiguratorIOLog | kIOPCIConfiguratorKPrintf); //Debug only
#endif
        next = NULL;
        for (child = entry->child; child; child = child->peer)
        {
            if (bus == entry->secBusNum)
            {
                ...
                if (!allocNeeded) continue;
                bridgeAllocateResources(child, kPCIHeaderType2);
                bridgeConnectDeviceTree(child);
            }
		}
#ifdef DEBUG
		fFlags &= ~(kIOPCIConfiguratorIOLog | kIOPCIConfiguratorKPrintf); //Debug only
...
//    if (bridge->subBusNum > 250) bridge->subBusNum = 52;
    ...
    if (OSDynamicCast(IOPCIDevice, bridge->dtNub)) printf("WARNING!: no host bridge\n");
    ...
    FOREACH_CHILD(bridge, child)
    {
        ...
        for (int i = kIOPCIRangeBridgeMemory; i < kIOPCIRangeCount; i++)
        {
            ...
            if (!(fFlags & kIOPCIConfiguratorAllocate))
            {
                ...
                if ((childRange->type == kIOPCIResourceTypeBusNumber) && (kIOPCIRangeCount < fWaitingPause))
                {
                    int addBusNum = fWaitingPause - kIOPCIRangeCount;
                    bridge->subBusNum = configRead8(bridge, kPCI2PCISubordinateBus);
                    configWrite8(bridge, kPCI2PCISubordinateBus, bridge->subBusNum + addBusNum);
                    IOLog("PCI configuration changed (%s free=%lx needed=%u added=%u)\n",
                          gPCIResourceTypeName[childRange->type],
                          (unsigned long)fWaitingPause, (unsigned int)kIOPCIRangeCount, addBusNum);
                }
            }
            // AnV - Fixes till here
		}
								        ...
								        uint32_t deallocTypes, uint32_t freeTypes)
{
    ...
    for (int rangeIndex = 0; rangeIndex < kIOPCIRangeCount; rangeIndex++)
	{
		...
		if ((kIOPCIRangeBridgeBusNumber == rangeIndex) && dead->busResv.nextSubRange)
		{
			...
			if (!ok) printf("WARNING!: IOPCIRangeListDeallocateSubRange busResv\n");
		}
...
void CLASS::deviceApplyConfiguration(IOPCIConfigEntry * device, uint32_t typeMask, bool dolog)
{
    ...
    // AnV - Additions
    UInt16 saved = configRead16(device, kIOPCIConfigCommand);
    // AnV - Till here
